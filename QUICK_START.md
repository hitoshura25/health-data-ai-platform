...
This will:
- ✅ Generate secure random credentials for all services
- ✅ Create coordinated `.env` files across all services
- ✅ Configure PostgreSQL with multiple databases (healthapi, webauthn)
- ✅ Set up MinIO, RabbitMQ, Redis with matching credentials
...
All services use a unified testing approach with the root-level `run-tests.sh` script.
...
**Note:** The project uses a single root-level virtual environment (`.venv`) shared by all services, not per-service environments.
...
└────────┬────────┘
         │
         ├──► WebAuthn Server ──► PostgreSQL (webauthn DB)
         │         │                 └──► Redis (sessions)
         │         │                 └──► Jaeger (tracing)
         │         ↓
         │    [JWT Token]
         │         ↓
         └──► Health API ──────────► PostgreSQL (healthapi DB)
                   │                 ├──► Redis (rate limiting)
                   │                 ├──► MinIO (file storage)
...
🔒 **The setup script generates random credentials every time it runs**
